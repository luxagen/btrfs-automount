    ...
    for (mount, cfg) in config_mut.iter_mut() {
        ...
        if let Some(sv_new) = find_latest(root_mount, &cfg.dir, &cfg.prefix)? {
            if let Some(blkdev) = root2bd.get(root_mount) {
                ...
                eprintln!(
                    "Warning: Could not find block device for root mount {}",
                    root_mount
                );
            }
        } else {
            eprintln!(
                "Warning: '{}' has no snapshots '{}/{}/{}.*'",
...
pub fn get_config(file_path: &str) -> Result<HashMap<String, ConfigEntry>> {
    let mut result = HashMap::new();

    let file = File::open(file_path).map_err(|_| BtramError::ConfigError)?;
    ...
    for line in BufReader::new(file).lines() {
        ...
        if !Path::new(mount).is_dir() {
            return Err(BtramError::BadConfig(format!(
        ...
        if !Path::new(root).is_dir() {
            return Err(
                BtramError::BadConfig(format!("Root '{}' is not a directory", root)).into(),
            );
        }
...
pub fn parse_mount(line: &str) -> Option<MountInfo> {
...
pub fn sv_info(root_mount: &str, sv: &str) -> Result<SubvolInfo> {
    ...
    if !output.status.success() {
        return Err(BtramError::CommandFailed(format!(
            "btrfs subvolume show failed for {}/{}",
            root_mount, sv
        ))
        .into());
    }
...
pub fn sv_impure(svi: &SubvolInfo) -> Option<&'static str> {
...
pub fn find_latest(root_mount: &str, ssdir: &str, ssn_prefix: &str) -> Result<Option<String>> {
    ...
    for entry in glob::glob(&glob_pattern)? {
        ...
        if let Some(caps) = re.captures(&file_name) {
            ...
            if let Some(status) = sv_impure(&sv_info) {
                eprintln!(
                    "Warning: ignoring {} subvol '{}/{}'",
                    status, ssdir, ssn_candidate
                );
                continue;
            }
...
pub fn process_mount(
    ...
    if let Some(sv) = sv {
        ...
        let root_mount = match bd2root.get(blkdev) {
            ...
            None => {
                eprintln!(
                    "Warning: {} has a subvol at '{}', but its root subvol is not mounted",
                    blkdev, mount
                );
                return Ok(None);
            }
        ...
        if let Some(sv_new) = find_latest(root_mount, ssdir, ss_prefix)? {
            if sv_new != sv {
                println!(
                    "'{}': '{}/{}' replaces '{}/{}'",
                    mount, root_mount, sv_new, root_mount, sv
                );
